...
String pin=request.getParameter("pincode");
	...
	pstmt = con
			.prepareStatement("update law_citizenreg set name=?,username=?,gender=?,date=?,contact=?,email=?,area=?,mandalam=?,district=?,state=?,country=?,pincode=? where uid=?");
	...
	int i = pstmt.executeUpdate();
				if (i > 0) {
					response.sendRedirect("CitizenHome.jsp?msg=success");
	}
%>
</body>
</html>